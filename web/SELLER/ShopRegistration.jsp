    ...
    <div class="flex flex-col">
        ...
        <main class="flex flex-col min-h-screen">
            ...
            <div class="flex flex-col flex-1 w-full h-full overflow-y-auto bg-gray-50 bg-opacity-50 items-center">
                <div class="px-6 py-6 mx-8 mt-4 text-left bg-white shadow-lg rounded-lg md:w-1/3 lg:w-1/3 sm:w-1/3">
                    <h3 class="text-2xl font-bold text-center mt-4">Set Up My Shop</h3>
                    <% 
                        String bidder_ID = request.getParameter("id");
                        sellerDAO sellerdao = new sellerDAOImpl();
                        Seller seller = sellerdao.retrieveSellerIDByBidderID(Integer.parseInt(bidder_ID));
                    %>
                    <form action="../sellerController" method="POST">
                        <div class="mt-8">
                            <div>
                                ...
                                <div >
                                    <label class="block" for="shopname">Shop Name<label>
                                        <input type="text" name="shopname" placeholder="Shop Name" class="w-full px-4 py-2 mt-2 border rounded-md focus:outline-none focus:ring-1 focus:border-yellow-100 focus:ring-yellow-300">
                                </div>
                                <div class="mt-8">
                                    <label class="block" for="shopdesc">Shop Description<label>
                                            <textarea placeholder="Shop Description" name="shopdesc" class="w-full px-4 py-2 mt-2 border rounded-md focus:outline-none focus:ring-1 focus:border-yellow-100 focus:ring-yellow-300"></textarea>
                                </div>
                            </div>
                            <div class="flex mt-3">
                                <button type="submit" name="command" value="saveShop" class="w-full px-6 py-2 mt-6 text-white rounded-lg bg-gray-600 hover:bg-gray-800">Next</button>
                            </div>
                        </div>
                    </form>
                </div>
