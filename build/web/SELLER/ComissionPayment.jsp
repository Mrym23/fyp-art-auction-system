<%-- 
    Document   : ComissionPayment
    Created on : Jun 28, 2022, 6:11:06 AM
    Author     : USER
--%>
            ...
            <div class="w-full container mx-auto flex flex-wrap bg-white border-b border-yellow-200 items-center justify-between mt-0 px-4 py-2">
                ...
                <div class="order-2 md:order-3 flex items-center" id="nav-content">
                    ...
                    <div class="dropdown group inline-block relative">    
                        ...
                        <ul class="dropdown-menu absolute hidden text-gray-700 pt-1" style="z-index:10;">
                            <li class=""><a class="rounded-t bg-gray-50 hover:bg-gray-100 py-2 px-4 block whitespace-nowrap" href="MyProfile.jsp?id=<%=seller.getBidderId()%>">My Profile</a></li>
                    ...
                    <div class="flex flex-col bg-white rounded-lg shadow-xl my-4 w-1/2 md:w-9/20">
                        <form action="../sellerController" method="POST">
                            <div class="flex justify-center">
                                <div class="flex pt-2">
                                    <h1 class="text-gray-600 font-bold md:text-2xl text-xl pt-4">Commission Payment</h1>
                                </div>
                            </div>
                            <div class="flex flex-col mx-7">
                                <input type="hidden" name="hidsellerid" value="<%=seller.getSellerId()%>" class="w-full px-4 py-2 mt-2">
                            </div>
                            <div class="flex flex-row">    
                                <div class="flex flex-col w-1/2 justify-center mt-7 mx-7">
                                    <label class="uppercase text-sm text-xs text-gray-500 text-light font-semibold">Total Commission:</label>
                                    <label class="pt-2 text-base text-gray-500 font-medium">RM<%=Comm%></label>
                                    <input type="hidden" name="Comm" value="<%=Comm%>" class="w-full px-4 py-2">
                                </div>
                                <div class="flex flex-col w-1/2 justify-center mt-7 mx-7">
                                    <label class="uppercase text-sm text-xs text-gray-500 text-light font-semibold">Month:</label>
                                    <label class="pt-2 text-base text-gray-500 font-medium"><%=currentMonth%></label>
                                </div>
                            </div>
                            ...
                            <div class="justify-center inline-block flex flex-col py-3 md:flex-row items-center ">
                                <button type="button" class="focus:outline-none inline-flex item-center text-white bg-yellow-400 hover:bg-yellow-500 focus:ring-4 focus:ring-yellow-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:focus:ring-yellow-900">
                                    <a href="Sales.jsp?id=<%=seller.getSellerId()%>">Back</a>
                                </button>
                                <button type="submit" name="command" value="commissionPayment" class="focus:outline-none inline-flex item-center text-white bg-yellow-400 hover:bg-yellow-500 focus:ring-4 focus:ring-yellow-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:focus:ring-yellow-900">
                                    Pay
                                </button>
                            </div>
                        </form>
